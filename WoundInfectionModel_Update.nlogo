...
breed [bacteria2 bacterium2]         ; this is the sub-category of turtles that are bacterial cells
...
turtles-own [
  age            ; a counter to keep track of turtles age after dividing (replaces "clock")
  prolif-rate    ; proliferation rate specific to each turtle
]
...
to Run-Simulation
  proliferate-bacteria1
  proliferate-bacteria2
  proliferate-neutrophils
  proliferate-macrophages
  migrate
  Leukocyte-Actions
  if predatory [predatory-bacteria-actions]

  if any? bacteria1 or any? bacteria2 [
  ...
  set number-of-cells count bacteria1 + count bacteria2         ; this counts the total number of bacteria cells, which is shown in the counter window below the main world.
  set clock clock + 1
  tick
...
to infect
  ask patch 0 0 [
    ask n-of Initial-Bacteria1 patches in-radius 15[
      ...
      [
        ...
        set age random prolif-rate1
        set prolif-rate prolif-rate1
      ]
    ]
  ]
 ask patch 0 0 [
    ask n-of Initial-Bacteria2 patches in-radius 15[
      sprout 1        ; create the number of initial bacteria as designated by the slider on the interface tab
      [
        set breed bacteria2
        ...
        set shape "bacteria"       ; denote the shape of the turtle as an bacteria cell shape
        set color green
        set age random prolif-rate2
        set prolif-rate prolif-rate2
      ]
    ]
  ]
...
to proliferate-bacteria1        ; this sub-routine simulates cell proliferation without any contact inhibition  [
  ask bacteria1 [
      if age mod prolif-rate = 0 and count bacteria1-on neighbors < 8                 ; cell division happens for every cell at the same time (i.e. clock tick) according to the 'prolif-rate' set on the interface tab. See 'proliferate' sub-routine below.
      [
        hatch 1 [
          let attempted-moves  0
          while [any? other bacteria1-here and attempted-moves < 16]
          [
            if not any? neighbors with [wound = 1 and not any? bacteria1-here][die]
            move-to one-of neighbors with [wound = 1 and not any? bacteria1-here]
...
to proliferate-bacteria2        ; this sub-routine simulates cell proliferation without any contact inhibition  [
  ask bacteria2 [
      if age mod prolif-rate = 0 and count bacteria2-on neighbors < 8                 ; cell division happens for every cell at the same time (i.e. clock tick) according to the 'prolif-rate' set on the interface tab. See 'proliferate' sub-routine below.
      [
        hatch 1 [
          let attempted-moves  0
          while [any? other bacteria2-here and attempted-moves < 16]
          [
            if not any? neighbors with [wound = 1 and not any? bacteria2-here][die]
            move-to one-of neighbors with [wound = 1 and not any? bacteria2-here]
            set attempted-moves attempted-moves + 1]
...
to migrate
  ...
  [
    ask bacteria1
    [
      if any? neighbors with [not any? bacteria1-here or any? bacteria2-here and wound = 1]            ; migration only occurs if there is at least one empty neighboring patch
      [
        move-to one-of neighbors with [not any? bacteria1-here or any? bacteria2-here] ; migrate to one of the 8 neighboring patches without a cell in it already
      ]
    ]
  ]
  ...
  [
    ask bacteria2
    [
      if any? neighbors with [not any? bacteria2-here or any? bacteria1-here and wound = 1]            ; migration only occurs if there is at least one empty neighboring patch
      [
        move-to one-of neighbors with [not any? bacteria2-here or any? bacteria1-here] ; migrate to one of the 8 neighboring patches without a cell in it already
      ]
    ]
  ]
  ...
  [
    ifelse any? bacteria1 in-radius Neutrophil-Homing-Range [
      face min-one-of bacteria1 [distance myself]
      fd 1
    ]
  ...
  ask macrophages
  [
    ifelse any? bacteria1-on neighbors or any? bacteria2-on neighbors or any? neighbors with [debris > 0]
    [move-to one-of neighbors with [any? bacteria1-here or any? bacteria2-here or debris > 0]]
    [move-to one-of neighbors]
  ]
end

to Leukocyte-Actions
  ask neutrophils [if any? bacteria1-on patch-here and random 100 <= kill-probability
    [
      ask patch-here [set debris debris + (count bacteria1-here)]
      ask bacteria1-on patch-here [die]
    ]

  ]

  ask neutrophils [if any? bacteria2-on patch-here
    [
      ask patch-here [set debris debris + (count bacteria2-here)]
      ask bacteria2-on patch-here [die]
    ]

  ]

  ask macrophages [if any? bacteria1-on patch-here and random 100 <= kill-probability
    [ask bacteria1-on patch-here [die]]
    if [debris] of patch-here > 0
    [ask patch-here [set debris debris - Macrophage-Strength]]
  ]

  ask macrophages [if any? bacteria2-on patch-here
    [ask bacteria2-on patch-here [die]]
    if [debris] of patch-here > 0
    [ask patch-here [set debris debris - Macrophage-Strength]]
  ]
...
to predatory-bacteria-actions
    ask bacteria1 [if any? bacteria2-on patch-here
    [
      ask patch-here [set debris debris + (count bacteria2-here)]
      ask bacteria2-on patch-here [die]
      hatch newBactAmount [
...
"" ""
PENS
"Bacteria #1" 1.0 0 -4079321 true "" "plot count bacteria1"
"Neutrophils" 1.0 0 -11221820 true "" "plot count neutrophils"
"Macrophages" 1.0 0 -10141563 true "" "plot count macrophages"
"Bacteria #2" 1.0 0 -7500403 true "" "plot count bacteria2"
"Total Bacteria" 1.0 0 -2674135 true "" "plot number-of-cells"
...
infect</setup>
    ...
    <timeLimit steps="200"/>
    <metric>number-of-cells</metric>
    <metric>count bacteria1</metric>
    <metric>count bacteria2</metric>
    ...
    <enumeratedValueSet variable="Prolif-rate1">
      <value value="12"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="Prolif-rate2">
      <value value="2"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="Initial-bacteria1">
      <value value="30"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="Initial-bacteria2">
      <value value="60"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="predatory">
      <value value="false"/>
      <value value="true"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="NewBactAmount">
      <value value="3"/>
    </enumeratedValueSet>
  </experiment>
  <experiment name="one cell experiment" repetitions="10" runMetricsEveryStep="true">
    <setup>setup
infect</setup>
    <go>run-simulation</go>
    <timeLimit steps="200"/>
  ...
  <experiment name="neutrophil experiment" repetitions="10" runMetricsEveryStep="true">
    <setup>setup
infect</setup>
    <go>run-simulation</go>
    <timeLimit steps="200"/>
  ...
  <experiment name="resistance experiment" repetitions="10" runMetricsEveryStep="true">
    <setup>setup
infect</setup>
    <go>run-simulation</go>
    <timeLimit steps="200"/>
